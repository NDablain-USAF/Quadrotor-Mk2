...
void Matrix_sub(float *A, float *B, float *C, uint8_t dim1, uint8_t dim2){
  // B is matrix to subtract from A, C is the resultant. dim1 and 2 are the dimensions of A and B
  for (uint8_t i=0;i<(dim1*dim2);i++){
    *C++ = *(A++)-*(B++);
  }
}
...
void Vector_dot(float *A, float *B, float *C, uint8_t length){
  // A and B are the vectors to take dot product of, C is the resultant. length is the length of A and B
  for (uint8_t i=0;i<length;i++){
    *C += *(A++)**(B++);
  }
}

void Vector_con(float *A, float *B, float *C, uint8_t length){
  // A is the vector to get multiplied, B is the constant to multiplt by, C is the resultant, length is the length of A
  for (uint8_t i=0;i<length;i++){
    *C++ = *(A++)**B;
  }
}
